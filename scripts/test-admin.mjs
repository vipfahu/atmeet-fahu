import {passwordHash} from '../lib/admin-password.ts';
...
const fetcher=async(url,options={})=>{
 ...
 if(p==='/rest/v1/rpc/admin_accept_invitation'){
  ...
  inv.used=true;created++;return ok([{...user,email:b.p_email,password_hash:b.p_password_hash}]);
 }
...
privileged=false;assert.equal((await handle(request('login',{email:user.email,password:'password'}))).status,401);privileged=true;
